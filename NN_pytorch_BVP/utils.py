...
def compute_grad(
    ...
    ----------
    y : torch.Tensor
        Output tensor to differentiate. This tensor is typically a scalar or
        otherwise compatible with ``torch.ones_like(y)`` as ``grad_outputs``.
    x : torch.Tensor
        Input tensor with respect to which the gradient is computed.
    create_graph : bool, default=True
        Whether to construct the graph of the derivative, enabling higher-order
        derivative computations.
    retain_graph : bool or None, default=None
        Whether to retain the computation graph after the backward pass. If
        ``None``, this is set to the value of ``create_graph``.
    ...
    Notes
    -----
    This function calls ``torch.autograd.grad`` with ``grad_outputs`` set to
    ``torch.ones_like(y)`` and returns the first gradient tensor.
...
def log_per_layer_ms(    # ms = mean square
    ...
    for j, layer in enumerate(logged_layers):
        ...
        weight_ms_history[log_idx, j] = _mean_square(layer.weight)
        if hasattr(layer, "bias") and layer.bias is not None:
            bias_ms_history[log_idx, j] = _mean_square(layer.bias)
        else:
            bias_ms_history[log_idx, j] = float("nan")

        # gradient mean squares
        if layer.weight.grad is not None:
            weight_grad_ms_history[log_idx, j] = _mean_square(layer.weight.grad)
        else: 
